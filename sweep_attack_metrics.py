...
def main():
    ...
    for removal_ratio in removal_ratios:
        ...
        with torch.no_grad():
            ...
            for image, _ in val_data:
                ...
                # For identity, attacked == encoded, so PSNR would be inf.
                # Store None for attack-only identity metrics instead.
                if args.attack != "identity":
                    ...
                    attack_ssim = compute_ssim(
                        encoded,
                        attacked,
                    )

                    attack_psnr_meter.update(
                        attack_psnr.item(),
                        current_batch_size,
                    )

                    attack_ssim_meter.update(
                        attack_ssim.item(),
                        current_batch_size,
                    )
                ...
                # ============================================
                #
                # Final evaluation uses batch size 1, so the
                # batch BER value is also the exact per-image
                # BER in that setting.
                # ============================================
                ...
                for local_idx in range(
                    ...
                    single_encoded = encoded[
                        local_idx:
                        local_idx + 1
                    ]

                    single_attacked = attacked[
                        local_idx:
                        local_idx + 1
                    ]
                    ...
                    if args.attack == "identity":
                        ...
                        single_attack_psnr = None
                        single_attack_ssim = None
                    ...
                    per_image_records.append(
                        {
                            ...
                            "psnr_attack": (
                                float(single_attack_psnr)
                                if single_attack_psnr is not None
                                else None
                            ),
                            "ssim_attack": (
                                float(single_attack_ssim)
                                if single_attack_ssim is not None
                                else None
                            ),
        ...
        results["results"][
            ...
            "psnr_attack": (
                float(attack_psnr_meter.avg)
                if args.attack != "identity"
                else None
            ),

            "ssim_attack": (
                float(attack_ssim_meter.avg)
                if args.attack != "identity"
                else None
            ),
        ...
        if args.attack != "identity":
            ...
            print(
                f"PSNR attack "
                f"(encoded -> attacked): "
                f"{attack_psnr_meter.avg:.4f}"
            )

            print(
                f"SSIM attack "
                f"(encoded -> attacked): "
                f"{attack_ssim_meter.avg:.4f}"
            )
    ...
    per_image_output_file = os.path.join(
        args.output_dir,
        per_image_output_filename,
    )
